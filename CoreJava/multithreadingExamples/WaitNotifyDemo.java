 ...
 * Such type of polling causes unnecessary burden on the machine hence we use wait and notify
 * method so that thread can communicate with each other and polling is avoided.
 * 
 * wait(): It is an object class method. When wait method is called on any thread then that thread
 *         will leave the resource and wait outside the object until it receives the notify method
 *         from other thread.
 * 
 * notify(): It is an object class method. When a thread calls notify method all the threads that 
 *           were waiting will become active again and will get ready to enter the object.
 */
class Warehouse{
